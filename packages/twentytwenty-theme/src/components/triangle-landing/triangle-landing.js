...
const TriangleLanding = ({ state, actions }) => {
  ...
  if (landingData.isReady) {
    ...
    const handleLoadedData = (type) => {
      console.log(`${type} video loaded`);
    };
    
    const foregroundVideo = item.foreground_media[0] && (
      <React.Fragment key={item.foreground_media[0].guid}>
        <VideoPlayerNative
          playsInline
          autoPlay
          muted
          loop
          onLoadedData={() => handleLoadedData("Foreground")}
        >
          <source
            src={item.foreground_media[0].guid + "#t=0.1"}
            type={"video/mp4"}
            key={item.foreground_media[0].guid}
          />
        </VideoPlayerNative>
      </React.Fragment>
    );
    const backgroundVideo = item.background_media[0] && (
      <BackgroundImageDiv>
        <React.Fragment key={item.background_media[0].guid}>
          <VideoPlayerNative
            playsInline
            autoPlay
            muted
            loop
            onLoadedData={() => handleLoadedData("Background")}
          >
            <source
              src={item.background_media[0].guid + "#t=0.1"}
              type={"video/mp4"}
              key={item.background_media[0].guid}
            />
          </VideoPlayerNative>
        </React.Fragment>
      </BackgroundImageDiv>
